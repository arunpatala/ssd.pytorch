...
class SSDAugmentation(object):
    def __init__(self, means=(104, 117, 123), dim=600):
        self.means = means
    ...
    def __call__(self, img, anno):
        ...
        boxes, labels = anno[:,:4], anno[:,4:]#torch.split(anno, 4, 1)

        #if boxes.dim() == 1:
        #    boxes.unsqueeze_(0)
        img, boxes,labels = self.augment(img, boxes, labels)
        return img, torch.from_numpy(np.concatenate([boxes, labels],axis=1))


"""The code that follows is a deprecated PIL Image implementation of both training
and testing data augmentation.  It is slow with large batch sizes, but works.
"""
